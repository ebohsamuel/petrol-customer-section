...
function UserMenu({ setIsShown, isShown, userRef, setShowVerticalNav, setActivePage }) {
  ...
  return (
    <>
        ...
        <div ref={userRef} className={"absolute right-[10px] h-[150px] bottom-[-145px] bg-white rounded-md shadow-xl z-10 flex flex-col gap-1 flex-none transition-opacity duration-200 ease-in-out " + (isShown ? "visible opacity-100" : "invisible opacity-0")}>
            ...
            <span onClick={handleAccountSettingClick} className="inline-block w-52 px-2 py-4 cursor-pointer font-[600] text-[18px] hover:bg-gray-200"><Link to="/account-setting">Account Settings</Link></span>
